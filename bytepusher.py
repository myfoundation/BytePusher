...
def update():
    ...
    start = mem[5] * 65536
    for i in range(W * H):
        b, g, r = COLORS[mem[start + i]].to_bytes(3, 'big')
        pixels[3 * i] = r
        pixels[3 * i + 1] = g
        pixels[3 * i + 2] = b
    label.image = tk.PhotoImage(width=W, height=H,
                                data=P6_HEADER + pixels,
                                format='PPM').zoom(SCALE, SCALE)
...
with open(sys.argv[1], 'rb') as f:
    ...
    mem.extend(bytearray(MEM_SIZE - len(mem)))
